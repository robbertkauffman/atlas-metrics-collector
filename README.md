...
1. Change the `CONFIG` variable in `get-atlas-node-metrics.py`:
    ...
    period and granularity of the metrics that are collected.
...
variables. Use ISO 8601-formatted date & time. The default is the last 24 hours 
...
5. Copy the output of the script and paste in a new Google Sheet.
    ...
    - Select all the CPU columns & rows (by default AY to BD) and navigate using 
    the main menu to: _Format > Number > Number_
